...
CPPFLAGS_QTMULTIMEDIA ?= $(shell PKG_CONFIG_PATH=$(PKG_CONFIG_PATH) $(PKGCONFIG) $(QT_PKG_PREFIX)Multimedia --cflags $(STDERR_TO_DEVNULL)) -DQT_NO_KEYWORDS
LIBS_QTMULTIMEDIA     ?= $(shell PKG_CONFIG_PATH=$(PKG_CONFIG_PATH) $(PKGCONFIG) $(QT_PKG_PREFIX)Multimedia --libs-only-L --libs-only-l $(STDERR_TO_DEVNULL))
...
dependencies-check:
	...
	checkheader $(QT_PKG_PREFIX)Multimedia QMediaPlayer QMediaPlayer::staticMetaObject "$(CPPFLAGS_QTMULTIMEDIA)" "$(LIBS_QTMULTIMEDIA)"; \
...
$(INSTALLDIR)/radiant.$(EXE): LIBS_EXTRA := $(LIBS_GL) $(LIBS_DL) $(LIBS_XML) $(LIBS_GLIB) $(LIBS_QTWIDGETS) $(LIBS_QTSVG) $(LIBS_QTNETWORK) $(LIBS_QTMULTIMEDIA) $(LIBS_ZLIB) $(LIBS_QTOPENGL) $(LIBS_QTOPENGLWIDGETS)
$(INSTALLDIR)/radiant.$(EXE): CPPFLAGS_EXTRA := $(CPPFLAGS_GL) $(CPPFLAGS_DL) $(CPPFLAGS_XML) $(CPPFLAGS_GLIB) $(CPPFLAGS_QTWIDGETS) $(CPPFLAGS_QTSVG) $(CPPFLAGS_QTNETWORK) $(CPPFLAGS_QTMULTIMEDIA) $(CPPFLAGS_QTOPENGL) $(CPPFLAGS_QTOPENGLWIDGETS) -Ilibs -Iinclude
$(INSTALLDIR)/radiant.$(EXE): \
	radiant/autosave.o \
