...
{
    bool texturename_ok = false;
    std::string texturename = string_copy_from_array_z(texture, &texturename_ok);

    if (!texturename_ok) {
        logging::print("WARNING: texture name '{}' was not zero-terminated\n", texturename);
    }

    return {vecs, {.native_q2 = static_cast<q2_surf_flags_t>(flags)}, -1, value, std::move(texturename), nexttexinfo};
}
