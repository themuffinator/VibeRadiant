...
- idTech2 compiler follow-up: backported stb_image's upstream bounded PNG-transparency loops to remove GCC's false out-of-bounds diagnostic, raised the reduced JsonCpp dependency's CMake policy floor to the enclosing toolchain baseline, and made `-version`/`--version` cleanly return success across all six command-line tools instead of falling through to an unknown-option error.
- idTech2 compiler suite refresh: advanced the reduced VibeyMapTools source snapshot from the v2.1.0-era `1028a4f4` revision to `4495049a` (`v3.1.0-1-g4495049a`), bringing in the upstream BSP, lighting, visibility, decompilation, format-handling, safety, and performance work; updated the production dependency subsets to fmt 12.1.0 and the upstream-pinned JsonCpp 1.9.6, removed the retired OptiX/cache/API sources, and preserved VibeRadiant's test/documentation pruning, deferred optional hub, C++17 JsonCpp baseline, and MSYS2 runtime/licence staging.
- Persistent asset-browser lifecycle safety: Entity and Object panes now invalidate their preview graphs, cached paths/categories, and raw resource pointers before entity-class, engine-path, VFS, or module teardown; readiness barriers and lifecycle revisions prevent queued filters or nested event processing from rebuilding against partially initialised dependencies. Live entity-definition, engine-path, game-name, game-mode, reference-refresh, and in-process restart requests are coalesced behind one prioritised idle gate, with serialised setting snapshots preserving the newest request across nested Qt event loops and shutdown cancellation beginning before browser widgets are destroyed. Module-backed graphs therefore cannot be freed or rebuilt from inside a preview/refresh event pump, replacement content appears only after every dependency is ready, explicit Object reloads rescan the post-refresh VFS, and hidden-pane scanning/model loading remains deferred until visible.
- Entity-browser map-transition recovery and scrolling: map teardown now blocks preview/filter reconstruction until the replacement scene is ready, then recomputes map-dependent `Used` results only when the Entity pane is visible (including an already-selected pane restored from a hidden outer panel). Hidden filter timers retain their pending work, list mode updates without loading GL previews, and incremental preview insertion/reference reloads no longer reset the scroll position on every observer callback; semantic category/filter changes perform one explicit reset instead.
- Reference-refresh browser consistency and efficiency: model-cache refreshes now suspend pending browser work before the wait dialog can process events, coalesce recursive refresh commands, defer requests that arrive during preview construction, and repair only preview graphs whose resources were actually reloaded. Object previews rebuild on the next visible event-loop turn, Entity previews reset their lazy-load count and repopulate only the visible viewport, pending filters survive the refresh, and hidden tabs no longer instantiate Entity models in the background.
- Missing-install startup efficiency: engine-path verification now reuses its freshly populated detected-install results when falling back from unique-match selection to first-valid auto-detection, instead of immediately repeating the same filesystem, Steam, GOG, and Windows uninstall-registry census. Independent setup-wizard detection still requests a fresh scan.
- Mapless diagnostic shutdown safety: `Map_Free` now tolerates runs that never captured a map resource, fixing the null dereference when `-startup-debug-mainwindow-only` exits. Last-map persistence is captured only from a valid loaded map before teardown, so a mapless diagnostic run no longer erases the user's previous reopen target.
- Model-browser map-change recovery: releasing a map now marks cleared object previews dirty and blocks their reconstruction until the replacement map and reference cache are ready. Visible previews repopulate at the safe post-load point, while hidden previews stay deferred until the Objects tab or its containing panel is shown, preventing both permanently blank previews and unnecessary hidden-tab model loading.
- Startup preference-dialog deferral: the complete Preferences UI is now constructed on first use instead of eagerly building every page and scanning detected game installations while the main window is still starting. Normal and search-targeted entry points share idempotent on-demand construction, shutdown remains safe when the dialog was never opened, and the in-process module refresh path first freezes the current preference callbacks to prevent duplicate or stale registrations from entering a later first build.
- Model-browser filter responsiveness: text edits now debounce expensive model-graph reconstruction into one apply after a short typing pause, while folder/global/used/clear actions remain immediate; reload, teardown, and reference-flush paths cancel pending work, switching away pauses it, and returning to Objects applies the retained filter before showing stale results. Filters that produce an unchanged visible model set preserve the existing graph, selection, transforms, and scroll position instead of synchronously unloading and reloading identical previews.
- Warm install performance and runtime-staging correctness: the incremental wrapper now preserves unchanged Meson-managed files only under a matching build/profile/Qt/toolchain install marker (tracked context switches and interrupted wrapper installs force a complete refresh), skips content-identical ericw copies, and validates content-addressed gamepack source/transform/output state before rebuilding the atomic gamepack tree. Windows runtime staging now resolves imports from the actually installed PE files in-process, selects the configured Qt major and active MSYS2 subsystem, distinguishes application-local DLLs, copies atomically only when content changed, and records a locked ownership/invalidation manifest instead of launching hundreds of `objdump`/`sed` processes and recopying over 220 MiB on every install; Meson and legacy Make `qt.conf`/plugin staging now match Qt 5/6. In the audited Qt 6 debug tree this reduced a measured 2m58.963s unchanged build-and-install baseline to an 8.03s three-run median (about 95.5%).
