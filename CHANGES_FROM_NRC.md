...
- Startup update responsiveness and safety: removed the automatic updater's synchronous pre-window network wait (two sequential requests with 20-second transfer timeouts where supported) and its obsolete nested-event-loop/early-quit plumbing; the existing preference- and interval-aware asynchronous check now starts only after the main window is shown and first-run setup/welcome dialogs have returned, keeping slow or unavailable networks off the time-to-interactive path. Reply cancellation now detaches completion handlers before aborting to avoid a synchronous-finish use-after-clear, and a manual check restarts any in-flight silent automatic lookup instead of appearing unresponsive.
- Incremental build/version performance: moved the Git suffix out of configure-time global compiler definitions into one build-time `vcs_tag`-generated PIC version object linked only to the binaries/plugins that display it, preserving legacy Make and non-Git base-version behavior while keeping build date/time coherent; ordinary VS Code build tasks now use a shared option- and source/toolchain-aware wrapper that reconfigures only for missing/stale Meson settings, rejects mismatched build contexts, and invokes the installed Ninja backend directly, eliminating a measured 3.024-second unconditional reconfigure and reducing a simulated post-commit build from 506 actions to 7 (about 98.6%).
- Nested ericw build performance and recovery: replaced the always-stale outer Meson target with a Ninja depfile that tracks the complete three-tool input graph (including compiler-discovered headers, PCH/object intermediates, CMake regeneration inputs, imported libraries, build metadata, toolchain executables, and final outputs), while retaining an always-check fallback for unsupported/incomplete graphs and non-Ninja backends; made configuration/build stamps atomic, expanded configuration identity to cover generator/toolchain details, pinned fresh Ninja-backend Meson sub-builds to Ninja without breaking legacy generator caches, and reduced the measured warm outer no-op median from 185.8 ms to 31.4 ms (about 83%).
- Asset-filter reset efficiency: Surfaces, AI, Script, and Object clear actions now update their backing/persisted state under scoped widget signal blockers, cancel any pending text-filter debounce, and perform exactly one immediate rebuild instead of re-entering toggle/text handlers and synchronously rescanning up to three times for one click.
- Asset-tree reload memory fix: Entity, Sound, and Object browsers now clear and reuse their existing Qt item models instead of retaining a complete parented model after every explicit reload; this also avoids unnecessary selection-model churn and clears the Entity browser's category pointer before replacing its backing category collection.
- Vendored compiler footprint/build hardening: reduced the embedded VibeyMapTools dependency snapshot by 645 tracked non-production files (16.05 MiB of Git payload), removing fmt/JsonCpp/pareto documentation, tests, examples, and bindings plus the entirely unused nanobench tree and its prebuilt benchmark binary; retained all production sources and required licence notices, disabled unavailable dependency features in the CMake cache, made the reduced snapshot's test/documentation constraints explicit, and verified clean/default production builds plus deterministic no-op restaging.
- Asset-browser lazy-cache follow-up: removed hidden-tab scene traversals that only initialized persisted Entity/Sound `Used` counts before first activation, and changed explicit Sound reload to clear cached audio and rebuild the VFS path tree without synchronously reading every sound back into memory; playback now repopulates the cache strictly on demand.
- Entity/Sound browser responsiveness and lifecycle hardening: coalesced scan-heavy text filtering behind cancellable 150 ms timers while keeping stored text/reset state immediate, made reset-all block its own widget signals and perform exactly one rebuild, cancelled delayed work across category/folder changes, reload, and teardown, cleared raw UI/media pointers during window destruction, and fixed Sound tree rebuilds so stale folder pointers and empty-result views cannot survive a reload.
