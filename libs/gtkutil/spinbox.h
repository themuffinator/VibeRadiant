...
template <class SpinT> class QSpinBox_mod : public SpinT {
  using ValueT =
      typename std::conditional_t<std::is_same_v<SpinT, QSpinBox>, int, double>;

public:
  QSpinBox_mod(ValueT min = 0, ValueT max = 99, ValueT value = 0,
               int decimals = 2, ValueT step = 1, bool wrap = false)
      : SpinT() {
...
protected:
  bool event(QEvent *event) override {
    if (event->type() == QEvent::ShortcutOverride) {
      ...
      if (keyEvent == QKeySequence::StandardKey::Undo ||
        ...
        return false;
      /* QAbstractSpinBox has no well defined ShortcutOverride routine, unlike
         underlying QLineEdit thus box' portion of key events ends up firing
         application's shortcuts (e.g. up, down, pgUp, pgDown) let's implement
         it kinda */
      if (keyEvent->key() == Qt::Key_PageUp ||
          keyEvent->key() == Qt::Key_PageDown ||
          keyEvent->key() == Qt::Key_Up || keyEvent->key() == Qt::Key_Down ||
          keyEvent->key() == Qt::Key_End || keyEvent->key() == Qt::Key_Home ||
  ...
  void wheelEvent(QWheelEvent *event) override {
    // dirty way to have substep modifiers w/o interaction with manipulated
    // values; only makes sense for DoubleSpinbox
    if (std::is_base_of_v<QDoubleSpinBox, SpinT> &&
...
template <typename SpinBoxT> class SpinBoxLabel : public QLabel {
protected:
  SpinBoxT *m_spin;
  bool m_isInDrag{};
  QPoint m_dragStart;
...
public:
  SpinBoxLabel(const QString &labelText, SpinBoxT *spin)
      : QLabel(labelText), m_spin(spin) {
...
protected:
  ...
  void mouseMoveEvent(QMouseEvent *event) override {
    if (m_isInDrag && event->buttons() == Qt::MouseButton::LeftButton) {
      m_dragAccum += event->globalPosition().toPoint().x() - m_dragStart.x();
      ...
      if (delta != 0) {
        ...
        // dirty way to have substep modifiers w/o interaction with manipulated
        // values; only makes sense for DoubleSpinbox
        if (std::is_base_of_v<QDoubleSpinBox, SpinBoxT> &&
            event->modifiers().testFlag(Qt::KeyboardModifier::ShiftModifier)) {
          const auto step = m_spin->singleStep();
          m_spin->setSingleStep(
              event->modifiers().testFlag(Qt::KeyboardModifier::ControlModifier)
                  ? step / 100
                  : step / 10);
          m_spin->stepBy(delta);
